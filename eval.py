...
def draw_label(image, point, ages, genders,font=cv2.FONT_HERSHEY_SIMPLEX,
    ...
    for i in range(len(point)):
        ...
        size = cv2.getTextSize(label, font, font_scale, thickness)[0]
        x, y = point[i]
        cv2.rectangle(image, (x+size[0], y + size[1]), (x + 2*size[0], y), (255, 0, 0), cv2.FILLED)
        cv2.putText(image, label, (x+size[0],y+size[1]), font, font_scale, (255, 255, 255), thickness)
...
if __name__ == '__main__':
    ...
    plt.imshow(aligned_image[0][:, :, (2, 1, 0)])
